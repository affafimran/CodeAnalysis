...
namespace _360Fabriek.Scenarios {
    public class TouchClipboard : MonoBehaviour {
        ...
        [SerializeField] private ClipboardTouchButton clipboardButtonPrefab;
        [SerializeField] private ClipboardStep clipboardStepPrefab;
        [SerializeField] private ClipboardTouchNavigation clipboardNavigationPrefab;

        private Vector3 startPosition;
        private Quaternion startRotation;
        private ClipboardTouchNavigation navigationInstance;
        ...
        public static TouchClipboard Instance { get; private set; }
        ...
        private void Start() {
            ...
            if (clipboardNavigationPrefab) {
                ...
                if (scenarioHolder)
                {
                   ScenarioManager.Instance.OnScenarioFinish.AddListener((Scenario _) => { navigationInstance.ShiftHolder(true, false); });
            ...
            if (vehicle && scenarioHolder) {
                ...
                for (int i = 0; i < vehicle.Scenarios.Count; i++) {
                    ...
                    ClipboardTouchButton clipboardButton = Instantiate(clipboardButtonPrefab, scenarioHolder);

                    clipboardButton.SetScenarioName(scenario.Name);
                    
                    if (clipboardButton.GetPressedAction())
                    {
                        OnClipboardButtonClicked(clipboardButton);
                    }
                }
            }
        }
        ...
        {
            ScenarioManager.Instance.OnScenarioFinish.AddListener((Scenario _) => { navigationInstance.ShiftHolder(true, false); });
            ScenarioManager.Instance.OnScenarioFinish.AddListener(AnimationOver.Instance.SetGameObject);
        }

        public void SetTransforms(bool stepHolder, bool scenarioHold)
        {
            stepsHolder.gameObject.SetActive(stepHolder);
            scenarioHolder.gameObject.SetActive(scenarioHold);
        }    

        public void OnClipboardButtonClicked(ClipboardTouchButton arg) {
            vehicle.SetScenarioByName(arg.name);
            if (navigationInstance) {
                SetTransforms(true, false);
                navigationInstance.ShiftHolder(false, true);
                navigationInstance.SetInteractability(navigationInstance.openStepHolder, true);
            }
        }

        public void OnVoicelineButtonClicked() {
        ...
        public void OnSetScenario(Scenario scenario) {
