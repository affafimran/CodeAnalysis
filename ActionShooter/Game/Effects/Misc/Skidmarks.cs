...
/// <summary>
/// Skidmarks.
/// Originally converted from SkidMarks.js. as foun on:
/// https://code.google.com/p/dorumon/source/browse/trunk/New+Unity+Project+4/Assets/scripts/Skidmarks.js?r=70
/// </summary>
public class Skidmarks : MonoBehaviour
{
	public int   maxMarks     = 1024;		// Maximum number of marks total handled by one instance of the script.
	public float markWidth    = 0.5f;		// The width of the skidmarks. Should match the width of the wheel that it is used for. In meters.
	...
	// Function called by the wheels that are skidding. Gathers all the information needed to
	...
	void LateUpdate()
	{
		...
		for (int i = 0; i < numMarks && i < maxMarks; i++)
		{
			if (skidmarks[i].lastIndex != -1 && skidmarks[i].lastIndex > numMarks - maxMarks)
			{
				...
				uvs[segmentCount * 4 + 0] = new Vector2(u, 0);
				uvs[segmentCount * 4 + 1] = new Vector2(u + 0.25f, 0);
				uvs[segmentCount * 4 + 2] = new Vector2(u, 1);
				uvs[segmentCount * 4 + 3] = new Vector2(u + 0.25f, 1);
